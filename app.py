...
def pg_cockpit(D, d0, d1):
    ...
    if todos:
        ...
        # ── Drill-down Faturamento × Arrecadação ─────────────────────────────
        with st.expander("🔍 Detalhamento — Faturamento por Componente e Bairro"):
            _meses_dd = todos if todos else []
            if _meses_dd:
                mes_dd = st.selectbox("Mês de referência", _meses_dd,
                                      index=len(_meses_dd)-1, key="dd_fat_mes")
                fat_dd = fat[pd.to_datetime(fat["dt_ref"]).dt.strftime("%m/%Y") == mes_dd] if not fat.empty else pd.DataFrame()
                if not fat_dd.empty:
                    # Componentes do faturamento
                    _comp_cols = {
                        "Água":           "vl_agua",
                        "Esgoto":         "vl_esgoto",
                        "Tarifa Básica Água": "vl_servico_basico_agua",
                        "Tarifa Básica Esgoto": "vl_servico_basico_esgoto",
                        "Lixo":           "vl_lixo",
                        "Serviços":       "vl_servico",
                    }
                    comp_vals = {k: fat_dd[v].sum() for k, v in _comp_cols.items() if v in fat_dd.columns and fat_dd[v].sum() > 0}
                    if comp_vals:
                        _dc1, _dc2 = st.columns([1, 1])
                        df_comp = pd.DataFrame(list(comp_vals.items()), columns=["Componente", "Valor"])
                        df_comp = df_comp.sort_values("Valor", ascending=True)
                        _cores_comp = ["#AED6F1","#5DADE2","#2E86C1","#1A5276","#85C1E9","#21618C"]
                        fig_dd1 = go.Figure(go.Bar(
                            x=df_comp["Valor"], y=df_comp["Componente"],
                            orientation="h",
                            marker_color=_cores_comp[:len(df_comp)],
                            text=df_comp["Valor"].apply(lambda v: f"<b>R$ {v:,.0f}</b>".replace(",",".")),
                            textposition="inside",
                            textfont=dict(size=12, color="white", family="Arial Black"),
                            insidetextanchor="end",
                        ))
                        fig_dd1.update_layout(
                            title=f"Componentes — {mes_dd}",
                            margin=dict(t=35, b=0, l=0, r=10), height=280,
                            xaxis=dict(title=""), yaxis=dict(title=""),
                        )
                        _dc1.plotly_chart(fig_dd1, use_container_width=True, key="dd_fig_comp")

                        # Top 10 bairros
                        if "d_bai" in D and not D["d_bai"].empty and "id_bairro" in fat_dd.columns:
                            fat_b = fat_dd.merge(D["d_bai"][["id_bairro","nm_bairro"]], on="id_bairro", how="left")
                            top_b = fat_b.groupby("nm_bairro")["vl_total_faturado"].sum()\
                                         .sort_values(ascending=True).tail(10).reset_index()
                            top_b.columns = ["Bairro", "Valor"]
                            _ne = len(top_b)
                            _gc = [f"rgb({int(26+i*(44-26)/max(_ne-1,1))},{int(82+i*(130-82)/max(_ne-1,1))},{int(118+i*(201-118)/max(_ne-1,1))})" for i in range(_ne)]
                            fig_dd2 = go.Figure(go.Bar(
                                x=top_b["Valor"], y=top_b["Bairro"],
                                orientation="h", marker_color=_gc,
                                text=top_b["Valor"].apply(lambda v: f"<b>R$ {v:,.0f}</b>".replace(",",".")),
                                textposition="inside",
                                textfont=dict(size=11, color="white", family="Arial Black"),
                                insidetextanchor="end",
                            ))
                            fig_dd2.update_layout(
                                title=f"Top 10 Bairros — {mes_dd}",
                                margin=dict(t=35, b=0, l=0, r=10), height=280,
                                xaxis=dict(title=""), yaxis=dict(title=""),
                            )
                            _dc2.plotly_chart(fig_dd2, use_container_width=True, key="dd_fig_bairro")

                    # Tabela resumo
                    vl_fat_dd = fat_dd["vl_total_faturado"].sum()
                    nr_eco_dd = fat_dd["nr_economia_agua"].sum() if "nr_economia_agua" in fat_dd.columns else 0
                    st.markdown(f"**Total faturado:** R$ {vl_fat_dd:,.2f}".replace(",","X").replace(".",",").replace("X",".") +
                                f" &nbsp;|&nbsp; **Economias:** {int(nr_eco_dd):,}".replace(",",".") +
                                (f" &nbsp;|&nbsp; **Média/Economia:** R$ {vl_fat_dd/nr_eco_dd:,.2f}".replace(",","X").replace(".",",").replace("X",".") if nr_eco_dd else ""),
                                unsafe_allow_html=True)
    else:
        st.info("Sem dados de faturamento/arrecadação no período.")

    # ── Drill-down SLA Serviços ───────────────────────────────────────────────
    if not srv.empty and "nm_setor_operacional" in srv.columns:
        with st.expander("🔍 Detalhamento — SLA por Setor Operacional"):
            _srv_ex = srv[srv["id_situacao_servico"] == 3] if "id_situacao_servico" in srv.columns else srv
            if not _srv_ex.empty:
                _ag_sla = _srv_ex.groupby("nm_setor_operacional").agg(
                    Total=("qt_servico","sum"),
                    ForaPrazo=("fl_fora_prazo", lambda x: (x == True).sum())
                ).reset_index()
                _ag_sla["No Prazo"] = _ag_sla["Total"] - _ag_sla["ForaPrazo"]
                _ag_sla["SLA%"] = _ag_sla["No Prazo"] / _ag_sla["Total"] * 100
                _ag_sla = _ag_sla.sort_values("Total", ascending=False).reset_index(drop=True)
                _cores_sla = [COR["verde"] if v >= 90 else COR["amarelo"] if v >= 75 else COR["vermelho"] for v in _ag_sla["SLA%"]]
                _fig_sla_dd = go.Figure()
                _fig_sla_dd.add_trace(go.Bar(
                    x=_ag_sla["nm_setor_operacional"], y=_ag_sla["SLA%"],
                    marker_color=_cores_sla,
                    text=_ag_sla["SLA%"].apply(lambda v: f"<b>{v:.1f}%</b>"),
                    textposition="inside", textangle=-90,
                    textfont=dict(size=13, color="white", family="Arial Black"),
                    insidetextanchor="middle",
                ))
                _fig_sla_dd.add_hline(y=90, line_dash="dash", line_color="gray", line_width=1.5,
                                       annotation_text="<b>Meta 90%</b>", annotation_position="top right",
                                       annotation_font=dict(size=11, color="gray"))
                _fig_sla_dd.update_layout(
                    title="% SLA no Prazo por Setor (apenas executados)",
                    margin=dict(t=50, b=10, l=0, r=10), height=320,
                    xaxis=dict(title=""), yaxis=dict(title="", ticksuffix="%", range=[0, 110]),
                    showlegend=False,
                )
                st.plotly_chart(_fig_sla_dd, use_container_width=True, key="dd_sla_setor")
                # Tabela resumo
                _ag_sla["SLA%"] = _ag_sla["SLA%"].round(1)
                st.dataframe(
                    _ag_sla[["nm_setor_operacional","Total","No Prazo","ForaPrazo","SLA%"]]\
                        .rename(columns={"nm_setor_operacional":"Setor","ForaPrazo":"Fora Prazo","SLA%":"SLA (%)"}),
